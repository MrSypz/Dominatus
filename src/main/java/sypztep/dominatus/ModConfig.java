...
public class ModConfig implements ConfigData {
    ...
    @Comment("Chunk search radius for finding nearby players (1 = 3x3 chunks, 2 = 5x5 chunks, etc.)")
    public static int mobScalingChunkRadius = 2;

    @ConfigEntry.Category("mob_scaling")
    @ConfigEntry.BoundedDiscrete(min = 0, max = 500)
    @Comment("Scaling factor as percentage (100 = 1:1 scaling, 50 = half scaling, 200 = double scaling)")
    public static int mobScalingFactorPercent = 100;

    @ConfigEntry.Category("mob_scaling")
    @ConfigEntry.BoundedDiscrete(min = 1, max = 10)
    @Comment("Minimum mob level (mobs will never go below this level)")
    public static int minMobLevel = 1;

    @ConfigEntry.Category("mob_scaling")
    @ConfigEntry.BoundedDiscrete(min = 10, max = 200)
    @Comment("Maximum level bonus from player scaling (caps how much mobs can be boosted)")
    public static int maxMobLevelBonus = 108;

    @ConfigEntry.Category("mob_scaling")
    @Comment("Apply dynamic scaling to boss mobs (Ender Dragon, Wither, etc.)")
    public static boolean scaleBossMobs = true;

    @ConfigEntry.Category("mob_scaling")
    @Comment("Apply dynamic scaling to passive mobs (cows, pigs, etc.)")
    public static boolean scalePassiveMobs = false;

    @ConfigEntry.Category("mob_scaling")
    @ConfigEntry.BoundedDiscrete(min = 0, max = 1000)
    @Comment("Block radius for alternative player detection method (0 = use chunk method)")
    public static int mobScalingBlockRadius = 0;

    // Helper methods for dynamic mob scaling
    public static float getMobScalingFactor() {
        return mobScalingFactorPercent / 100.0f;
    }

    public static boolean shouldScaleMob(boolean isBoss, boolean isPassive) {
        if (!enableDynamicMobScaling) {
            return false;
        }

        if (isBoss && !scaleBossMobs) {
            return false;
        }

        if (isPassive && !scalePassiveMobs) {
            return false;
        }
